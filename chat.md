...
_ℹ️ Note: you <b>need</b> to create a new account even if you already have a matrix account on another server! If you do, create a new account on the archeo.social server via [https://matrix.to/#/#isba:archaeo.social](https://matrix.to/#/#isba:archaeo.social)_.
...
Please check that your account is indeed on archaeo.social server not on any other matrix server. You can see that by checking your profile (left top, you click on your profile picture > All settings > Account): @<i>username</i>:archaeo.social (NOT for example @<i>username</i>:matrix.org). If you have problems communicating with somebody else, it is possible that they are on a wrong server: please let them know via other means.